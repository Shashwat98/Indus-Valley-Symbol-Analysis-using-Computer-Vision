# src/run_extract_features_rich.py
import csv
...
from .features import extract_features_rich
...
def run_feature_extraction_rich(symbol_index_csv: Path) -> None:
    """
    Main entry point for rich feature extraction (Feature Set B).
    """
    ...
    for i, row in df.iterrows():
        ...
        feat_vec, feat_dict = extract_features_rich(views)
    ...
    # Save CSV
    out_csv = FEATURES_DIR / "features_rich.csv"
    ...
    print(f"Wrote rich feature table to {out_csv}")

    # Save numeric matrix (.npy)
    X = np.stack(feature_matrix, axis=0)
    out_npy = FEATURES_DIR / "features_rich.npy"
    np.save(out_npy, X)
    print(f"Wrote rich feature matrix {X.shape} to {out_npy}")


if __name__ == "__main__":
    symbol_index_path = FEATURES_DIR / "symbol_index.csv"
    run_feature_extraction_rich(symbol_index_path)
